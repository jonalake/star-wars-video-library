...
fetch(`https://swapi.dev/api/films/${queryString.get("movie")}`)
    ...
    .then(response => {
        ...
        if (response.title === "A New Hope") {
            fetch(`https://www.omdbapi.com/?t=Star+Wars&y=&apikey=cba21048`)
                .then(response => response.json())
                .then(response => {
            ...
            fetch(`https://www.omdbapi.com/?t=${response.title}&y=${response.release_date.slice(0, 4)}&apikey=cba21048`)
                .then(response => response.json())
                .then(response => {
                    const poster = document.querySelector(".poster")
                    poster.src = `${response.Poster}`
